...
interface SidebarProps {
  activeTab: string;
  // Añadimos 'settings' a las opciones permitidas
  setActiveTab: (tab: 'dashboard' | 'tasks' | 'calendar' | 'analytics' | 'settings') => void;
}
  ...
  return (
    <aside className="hidden md:flex w-72 bg-white border-r border-slate-200 flex-col fixed h-full z-10">
      ...
      <div className="p-4 border-t border-slate-100">
        {/* Botón Configuración ahora interactivo */}
        <button 
        ...
        <div className="flex items-center gap-3 mt-4 px-4 py-2">
          <div className="w-9 h-9 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 shadow-sm flex items-center justify-center text-white font-bold text-sm">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-xs truncate ${isPremium ? 'text-indigo-600 font-bold' : 'text-slate-400'}`}>
               {isPremium ? 'Estudiante Pro ⚡' : 'Plan Básico'}
            </p>
          </div>
        </div>
      </div>
